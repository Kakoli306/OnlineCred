    ...
    <div class="iq-card-body">
        <div class="d-flex justify-content-between mb-3">
            ...
            <div class="align-self-center">
                ...
                @else
                    <a href="{{route('account.manager.providers.list',0)}}" class="btn btn-sm btn-primary">
                        <i class="ri-arrow-left-circle-line"></i>Back
                    </a>
                @endif
        ...
        <div class="d-lg-flex">
            <div class="client_menu mr-2">
                <ul class="nav flex-column setting_menu">
                    ...
                    <!--/ Profile Picture -->
                    <li class="nav-item">
                        <a class="nav-link active" href="{{route('account.manager.provider.info',$provider->id)}}">Provider
                            Info</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="{{route('account.manager.provider.contract',$provider->id)}}">Contracts</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="{{route('account.manager.provider.document',$provider->id)}}">Documents</a>
                    </li>
                    <li class="nav-item">
                    ...
                    <li class="nav-item">
                        <a class="nav-link"
                           href="{{route('account.manager.provider.activity',$provider->id)}}">Provider
                            Activity</a>
                    </li>
                </ul>
            </div>
            <form action="{{route('account.manager.provider.info.update')}}" method="post" enctype="multipart/form-data"
    ...
    <script>
        $(document).ready(function () {
            $('.existsphonedelete').click(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{route('account.manager.delete.exist.provider.phone')}}",
                    data: {
                        '_token': "{{csrf_token()}}",
                        'phonid': phonid
                    },
            ...
            $('.deleteexistsemail').click(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{route('account.manager.delete.exist.provider.email')}}",
                    data: {
                        '_token': "{{csrf_token()}}",
                        'emailid': emailid
                    },
            ...
            $('.deleteexistsaddress').click(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{route('account.manager.delete.exist.provider.address')}}",
                    data: {
                        '_token': "{{csrf_token()}}",
                        'addressid': addressid
                    },
